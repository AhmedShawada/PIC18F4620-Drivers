...
static volatile uint8 i2c_slave_2_REC_Data;
...
int main()
{
    ...
    while (1)
    { 
        if('a' == i2c_slave_2_REC_Data)
        {
            ret = dc_motor_move_right(&dc_motor_1);
        }
        else if('c' == i2c_slave_2_REC_Data)
        {
...
void MSSP_I2C_DefaultInterruptHandler (void)
{
    ...
    if((SSPSTATbits.R_nW == 0) && (SSPSTATbits.D_nA == 0))
    {
        ...
        ret = MSSP_I2C_Master_Read_Blocking(&i2c_obj,ACK,&i2c_slave_2_REC_Data);
        ret = lcd_4bits_send_char_data(&lcd_1,i2c_slave_2_REC_Data);
