 ...
 * Author: ashaw
 *
 * Created on July 12, 2024, 10:47 PM
 */

#include "hal_gpio.h"


/* Referance to the Data Direction Control Register */
volatile uint8 *tris_register[] = {&TRISA ,&TRISB ,&TRISC ,&TRISD ,&TRISE};
/* Referance to the Data Latch Register (Read and Write to Data Latch */
volatile uint8 *lat_register[] = {&LATA ,&LATB ,&LATC ,&LATD ,&LATE};
/* Referance to the Port Status Register*/
...
{ 
    ...
    {
        TOOGGLE_BIT(*lat_register[_pin_config->port], _pin_config->pin);
    }
